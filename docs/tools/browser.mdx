...
<Tabs>
  <Tab title="CLI 安装（推荐）">
    ...
    - 自动检测国内网络并使用镜像加速，镜像下载失败时自动回退官方源
  </Tab>
  <Tab title="手动安装">
    ```bash
    ...
    手动安装时建议优先使用清华源；若下载 `playwright` wheel 时返回 `403/404`，再切回官方源：

    ```bash
    PYTHONNOUSERSITE=1 pip install playwright -i https://pypi.tuna.tsinghua.edu.cn/simple
    python -m playwright install chromium
    ```

    ```bash
    PYTHONNOUSERSITE=1 pip install playwright --index-url https://pypi.org/simple
    python -m playwright install chromium
    ```
