...
| `feishu_reply_mode` | Reply mode. `stream_card` is the default streaming Markdown card, `normal` uses regular messages |

If your runtime sets `HTTP_PROXY`, `HTTPS_PROXY`, or `ALL_PROXY` to a SOCKS endpoint such as `socks5://...`, install the extra dependency below before starting the Feishu websocket client:
